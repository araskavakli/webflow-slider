...
(() => {
  // bin/live-reload.js
  new EventSource(`http://localhost:${4e3}/esbuild`).addEventListener(
    "change",
    () => location.reload()
  );
  ...
  var swipers = /* @__PURE__ */ new Map();
  var colors = ["#22c55e", "#eab308", "#ec4899"];
  ...
  function swiperInit() {
    swiperContainers.forEach((el, idx) => {
      ...
      if (window.innerWidth < disableSwiperAboveThis && !swiperIsOn) {
        ...
        const swiper = new core_default(el, {
        ...
        debugMode && console.log(
          `%c Swiper${swiperContainers.length > 1 ? " #" + Number(idx + 1) : ""} is ON. Window size < ${disableSwiperAboveThis}px.`,
        ...
        debugMode && console.log(
          `%c Swiper${swiperContainers.length > 1 ? " #" + Number(idx + 1) : ""} is OFF. Window size > ${disableSwiperAboveThis}px.`,
          `background: #222; color: ${colors[idx] || colors[idx % colors.length]}`
        );
      } else if (!swiperIsOn && window.innerWidth > disableSwiperAboveThis) {
        [...el.querySelectorAll(".swiper-slide")].forEach((slide) => {
  ...
  function debounce(func) {
    ...
    return function(event2) {
      if (timer)
        clearTimeout(timer);
      timer = setTimeout(func, 100, event2);
    };
  }
